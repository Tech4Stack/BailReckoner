...
class IKApi:
    def __init__(self, args, storage):
        ...
        self.headers = {'Authorization': 'Token %s' % args.token, 'Accept': 'application/json'}
        self.basehost = 'api.indiankanoon.org'
        self.storage = storage
        
        # Ensure maxcites and maxcitedby are initialized properly
        self.maxcites = args.maxcites if args.maxcites is not None else 0
        self.maxcitedby = args.maxcitedby if args.maxcitedby is not None else 0
        self.orig = args.orig
    ...
    def save_search_results(self, q):
        ...
        pagenum = 0
        current = 1
        docids = set()  # Use a set to avoid duplicate docid entries
        while True:
            results = self.search(q, pagenum, self.maxpages)
            obj = json.loads(results)
            
            # Debug: Print the entire response
            print("API Response:", obj)

            if 'docs' not in obj:
                self.logger.error("API Response does not contain 'docs': %s", obj)
                break
            ...
            for doc in docs:
                ...
                if self.download_doc(docid, docpath):
                    docids.add(docid)  # Mark as processed
